/**
 * Simple Demo Test for MCP Data Layer
 * 
 * This is a simplified version to test our enhanced extraction capabilities
 * without TypeScript compilation issues.
 */

// Simple interfaces for demo
interface SimpleFigmaData {
  id: string;
  name: string;
  type: string;
  children?: SimpleFigmaData[];
}
...
class SimpleDemoExtractor {
   ...
   */
  async extractDemo(figmaFileKey: string, nodeIds?: string[]): Promise<DemoResult> {
   ...
   */
  private generateDemoFrames(): any[] {
    return [
      {
        id: 'frame-1',
   ...
   */
  private generateDemoDesignSystemLinks(): any {
    return {
      buttons: 'design-system/buttons',
      typography: 'design-system/typography',
      colors: 'design-system/colors',
   ...
   */
  private generateDemoComponentInstances(): any[] {
    return [
      {
   ...
   */
  private countTotalLayers(frames: any[]): number {
    let count = 0;
   ...
   */
  private countLayersInLayer(layer: any): number {
    let count = 0;
 ...
 */
async function runMCPDemo(figmaFileKey: string, nodeIds?: string[]): Promise<DemoResult> {
